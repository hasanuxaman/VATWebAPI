...
{
    [RoutePrefix("api/ProductionIssues"), Authorize]
    public class ProductionIssuesController : ApiController
    {
        private readonly IMapper _mapper;
        private AuthHelper _authHelper = new AuthHelper();
        private APIUtilities APIUtilities = new APIUtilities();

        public ProductionIssuesController(IMapper mapper)
        {
            _mapper = mapper;
        }

        [HttpPost, Route("Import")]
        public RequestResult Import(ProductionIssueList vms)
        {
            ...
            try
            {
                ...
                if (vms.Invoices != null)
                {
                    ...
                    ProductionIssuesModel _vm = new ProductionIssuesModel();
              ...
              .ToList();
                    ...
                    {
                        ProductionIssuesRepo Repo = new ProductionIssuesRepo(_mapper);
                        user.BIN = vms.Invoices.FirstOrDefault().Invoice.BIN;

                        APIUtilities.Login(user);

                         dataTable = APIUtilities.ProductionIssueConvertToDataTable(vms.Invoices);
