...
{
    [RoutePrefix("api/Customers"), Authorize]
    public class CustomersController : ApiController
    {
        ...
        public RequestResult Import(CustomerList vms)
        {
            ...
            try
            {
                if (vms.customers != null)
                {
                    LogInModel user = new LogInModel();

                    var modelsWithDifferentCompanyCode = vms.customers
              .Where(customer => customer.BIN != bin)
              .ToList();
                    if (modelsWithDifferentCompanyCode.Count > 0)
                    {
                        throw new Exception("Wrong Company Code Given for this Customer Name: " + modelsWithDifferentCompanyCode.FirstOrDefault().CustomerName);
                    }

                    foreach (var customer in vms.customers)
                    {
                        CustomersRepo Repo = new CustomersRepo(_mapper);
                        var _vm = _mapper.Map<CustomerDto, CustomersModel>(customer);
                        if (string.IsNullOrEmpty(customer.BIN))
                        {
                            throw new Exception("Company Code Not Given for this customer Name: " + customer.CustomerName);
                        }
                        else
                        {
                            user.BIN = customer.BIN;
